...
def test_bin_negbinom(intv_bin_ip, intv_bin_con, alpha_ip_vec, alpha_con_vec, correction_method='fdr_bh'):
	"""DOCSTRING
	Args
	Returns
	"""
...
def gene_to_count(bam_dict, gene, unique_only=False, with_control=False, winsize=50, unstranded=False):
	"""DOCSTRING
	Args
	Returns
	"""
	...
	bin_interval_ip = bin_interval_counts(interval_ip, winsize=winsize)
	bin_interval_con = bin_interval_counts(interval_con, winsize=winsize)
	
	return bin_interval_ip, bin_interval_con
	
	
def test_gene_bin(gene, bin_interval_ip, bin_interval_con, alpha_ip_vec, alpha_con_vec):
	# perform statistical test
	signal_val, binscore_adj = test_bin_poisson(bin_interval_ip, bin_interval_con)
...
def estim_dispersion_param(alpha_len, gene_count, type):
	"""DOCSTRING
	Args
	Returns
	"""
	alpha_vec = np.zeros(alpha_len)
	for i in range(len(alpha_ip_vec)):
		dist = defaultdict(int)
		for gene_name in gene_count:
			this_height = ztnb_em.collapse_data(gene_count[gene_name][type][i,])
			for h in this_height:
				#if h==0 or h>50:
				#	continue
				dist[int(h)] += this_height[h]
		dist[0] = 0 ## truncate all zeros
		ll, mu, alpha = EM_estim_params(dist, max_iter=1000, verbose=True)
		alpha_vec[i] = alpha
	return alpha_vec
