...
def main():
	...
	if not os.path.isfile(output_dir + '/repeat_mapper.sorted.out'):
		subprocess.call(''' sort -k1,1 -k2,2n %s/repeat_mapper.out | awk '{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6}' > %s/repeat_mapper.sorted.out ''' % (output_dir, output_dir), shell=True)
	# Note: tid_list: tid -> [chr:strand, start, end]
	tid_list=read_aligner_output(output_dir + '/repeat_mapper.sorted.out', options.gtf, options.is_stranded, tmp_dir, options.resume, options.call_all)
	...
	if not (options.resume and os.path.isfile(tmp_dir+'/unique_to_qval.pdata') and os.path.isfile(tmp_dir+'/combined_to_qval.pdata')):
		...
		tid_to_qval_compact = pool.map(get_permutation_fdr, [ (unibam_file, multibam_file, tid_list, child_transcr_ind[i], options.pval_cutoff, options.max_iter, options.is_stranded, verbose, options.correction_method) for i in range(options.nb_proc) ])
...
def get_permutation_fdr((unibam_file, multibam_file, tid_list, tid_ind, pval_cutoff, max_iter, is_stranded, verbose, correction_method)):
	"""
